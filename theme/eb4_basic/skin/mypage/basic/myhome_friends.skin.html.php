<?php
/**
 * skin file : /theme/THEME_NAME/skin/mypage/basic/myhome_friends.skin.html.php
 */
...
<div class="myhome-follow">
    <div class="follow-heading">
        ...
        <strong>
            <?php if ($user['mb_id'] == $member['mb_id']) { ?>나의 맞팔친구<?php } else { ?><?php echo $user['mb_nick']; ?> 님의 맞팔친구<?php } ?>
            <small class="font-normal margin-left-5">( <span class="color-green"><?php if ($user['cnt_friends']) { ?><?php echo number_format($user['cnt_friends']); ?><?php } else { ?>0<?php } ?>명</span> )</small>
        </strong>
    </div>
    <div class="infinite-container">
                ...
                <div class="follow-info">
                    ...
                    <div class="follow-info-content">
                        ...
                        <span class="font-size-12">
                            팔로워 : <span class="margin-right-10"><?php echo number_format($list[$i]['follower']); ?></span>
                            팔로윙 : <span><?php echo number_format($list[$i]['friends']); ?></span>
                        </span>
                        ...
                        <div id="follow_<?php echo $list[$i]['mb_id']; ?>" class="profile-btns">
                            ...
                            <?php if ($list[$i]['mb_id'] == $member['mb_id']) { ?>
                                <button type="button" class="btn-e btn-e-red">It's Me! <i class="fas fa-child"></i></button>
                            <?php } else { ?>
                                <?php if (!$eb->follow_check($list[$i]['mb_id'])) { ?>
                                <button type="button" class="btn-e btn-e-yellow follow_<?php echo $list[$i]['mb_id']; ?>" name="<?php echo $list[$i]['mb_id']; ?>" value="follow" title="친구맺기를 신청합니다."><i class="fas fa-check"></i> 팔로우</button>
                                <?php } else { ?>
                                <button type="button" class="btn-e btn-e-dark follow_<?php echo $list[$i]['mb_id']; ?>" name="<?php echo $list[$i]['mb_id']; ?>" value="unfollow" title="친구관계를 해제합니다."><i class="fas fa-times"></i> 팔로우 취소</button>
                                <?php } ?>
                                <div class="clearfix"></div>
                            <?php } ?>
                            </a>
        ...
        <div class="text-center color-grey font-size-14 margin-top-30">
            <i class="fas fa-exclamation-circle"></i> 맞팔친구가 없습니다.
        </div>
        <?php } ?>
    </div>
    <div id="infinite_pagination">
        <a class="next" href="<?php echo G5_URL; ?>/?<?php echo $user['mb_id']; ?>&friends&page=<?php echo ($page+1); ?>"></a>
    </div>
</div>
