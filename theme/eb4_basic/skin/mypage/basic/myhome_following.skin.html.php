<?php
/**
 * skin file : /theme/THEME_NAME/skin/mypage/basic/myhome_following.skin.html.php
 */
...
<div class="myhome-follow">
    <div class="follow-heading">
        ...
        <strong>
            <?php if ($user['mb_id'] == $member['mb_id']) { ?>나의 팔로잉<?php } else { ?><?php echo $user['mb_nick']; ?> 님의 팔로잉<?php } ?>
            <span class="font-normal m-l-5">[ <span class="text-blue"><?php if ($user['cnt_following']) { ?><?php echo number_format($user['cnt_following']); ?><?php } else { ?>0<?php } ?>명</span> ]</span>
        </strong>
    </div>
    <div class="infinite-container">
            ...
            <section class="follow-item-box">
                ...
                <div class="follow-info">
                    ...
                    <div class="follow-info-content">
                        ...
                        <div class="margin-hr-15"></div>
                        <div class="follow-sign">서명 : <?php if($list[$i]['mb_signature']) { ?><?php echo stripslashes($list[$i]['mb_signature']); ?><?php } else { ?><span class="text-light-gray">입력한 서명이 없습니다.</span><?php } ?></div>
                        <?php } ?>
                        <?php if ($config['cf_use_profile'] == '1') { ?>
                        <div class="margin-hr-15"></div>
                        <div class="follow-introduce">소개 : <?php if($list[$i]['mb_profile']) { ?><?php echo stripslashes($list[$i]['mb_profile']); ?><?php } else { ?><span class="text-light-gray">입력한 소개가 없습니다.</span><?php } ?></div>
                        <?php } ?>
                        ...
                        <div id="follow_<?php echo $list[$i]['mb_id']; ?>" class="profile-btns">
                            ...
                            <?php } else { ?>
                                ...
                                <?php } else { ?>
                                <button type="button" class="btn-e btn-e-gray follow_<?php echo $list[$i]['mb_id']; ?>" name="<?php echo $list[$i]['mb_id']; ?>" value="unfollow" title="친구관계를 해제합니다."><i class="fa fa-times m-r-5"></i>팔로우 해제</button>
                                <?php } ?>
                                <div class="clearfix"></div>
                            <?php } ?>
                            </a>
        ...
        <div class="text-center text-gray m-t-30">
            <i class="fa fa-exclamation-circle"></i> 팔로잉한 회원이 없습니다.
        </div>
        <?php } ?>
    </div>
    <div id="infinite_pagination">
        <a class="next" href="<?php echo G5_URL; ?>/?<?php echo $user['mb_id']; ?>&following&page=<?php echo ($page+1); ?>"></a>
    </div>
</div>
