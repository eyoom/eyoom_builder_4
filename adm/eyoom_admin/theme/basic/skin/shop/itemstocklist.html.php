 ...
 * @file    ~/theme/basic/skin/shop/itemstocklist.html.php
 */
...
<div class="alert alert-info">
    <p>
        <i class="fas fa-info-circle"></i> 재고수정의 수치를 수정하시면 창고재고의 수치가 변경됩니다.
    </p>
</div>
...
!function () {
    ...
    db.clients   = [
        <?php for ($i=0; $i<count((array)$list); $i++) { ?>
        {
            ...
            창고재고: "<?php echo $list[$i]['it_stock_qty_str']; ?>",
            주문대기: "<?php echo number_format($list[$i]['wait_qty']); ?>",
            가재고: "<?php echo number_format($list[$i]['temporary_qty']); ?>",
            재고수정: "<label class='input'><input type='text' name='it_stock_qty[<?php echo $i; ?>]' id='stock_qty_<?php echo $i; ?>' value='<?php echo $list[$i]['it_stock_qty']; ?>' autocomplete='off'></label>",
            통보수량: "<label class='input'><input type='text' name='it_noti_qty[<?php echo $i; ?>]' id='noti_qty_<?php echo $i; ?>' value='<?php echo $list[$i]['it_noti_qty']; ?>' autocomplete='off'></label>",
            ...
            재고입고알람: "<label for='stock_sms_<?php echo $i; ?>' class='checkbox'><input type='checkbox' name='it_stock_sms[<?php echo $i; ?>]' id='stock_sms_<?php echo $i; ?>' value='1' <?php echo $list[$i]['it_stock_sms'] ? 'checked':''; ?>><i></i></label>",
        },
        <?php } ?>
    ]
}();
...
$(document).ready(function(){
    $("#admin-shop-itemstocklist").jsGrid({
        ...
        fields         : [
            ...
            { name: "재고입고알람", type: "text", align: "center", width: 100 },
        ]
    })
    $("#sort").click(function() {
        var field = $("#sortingField").val();
        $("#admin-shop-itemstocklist").jsGrid("sort", field);
    });
});
