 ...
 * @file    ~/theme/basic/skin/theme/ebslider_list.html.php
 */
...
<div class="admin-ebslider-list">
    <div class="adm-headline adm-headline-btn">
        <h3>EB슬라이더 - 마스터관리</h3>
        <?php if (!$wmode) { ?>
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=theme&pid=ebslider_form&amp;thema=<?php echo $this_theme; ?>" class="btn-e btn-e-red btn-e-lg"><i class="fas fa-plus"></i> EB슬라이더 마스터추가</a>
    ...
    <form name="febsliderform" id="febsliderlist" action="<?php echo $action_url1; ?>" method="post" onsubmit="return febsliderform_submit(this);" class="eyoom-form">
        ...
        <div class="col col-12">
            <div class="padding-top-5 clearfix">
                <span class="font-size-12 color-grey">
                    <a href="<?php echo G5_ADMIN_URL; ?>/?dir=<?php echo $dir; ?>&amp;pid=<?php echo $pid; ?>">[전체목록]</a><span class="margin-left-10 margin-right-10 color-light-grey">|</span>EB슬라이더 마스터 <?php echo number_format($total_count); ?>개
                </span>
            </div>
        </div>
    </div>
...
!function () {
    ...
    db.clients   = [
        <?php for ($i=0; $i<count($list); $i++) { ?>
        {
            ...
            관리: "<a href='<?php echo G5_ADMIN_URL; ?>/?dir=theme&amp;pid=ebslider_form&amp;thema=<?php echo $this_theme; ?>&amp;es_code=<?php echo $list[$i]['es_code']; ?>&amp;w=u'><u>수정</u></a>",
            슬라이더마스터제목: "<?php echo get_text($list[$i]['es_subject']); ?>",
            치환코드: "<div class='eb-clipboard'><div id='subs_code' class='eb-clipboard-cont'><?php echo $list[$i]['es_chg_code']; ?></div><div class='eb-clipboard-btn' data-clipboard-target='#subs_code'>코드복사</div></div>",
            상태: "<label for='es_state_<?php echo $i; ?>' class='select'><select name='es_state[<?php echo $i; ?>]' id='es_state_<?php echo $i; ?>'><option value=''>선택</option><option value='1' <?php echo  $list[$i]['es_state'] == '1' ? 'selected':''; ?>>보이기</option><option value='2' <?php echo  $list[$i]['es_state'] == '2' ? 'selected':''; ?>>숨기기</option></select><i></i></label>",
            등록일: "<?php echo substr($list[$i]['es_regdt'], 0, 10); ?>",
        },
        <?php } ?>
    ]
}();
...
$(function() {
    $("#ebslider-list").jsGrid({
        ...
        fields         : [
            ...
            { name: "슬라이더마스터제목", type: "text", width: 200 },
