...
!function () {
    ...
    db.clients   = [
	    <?php for ($i=0; $i<count((array)$list); $i++) { ?>
        {
	        ...
	        링크_타겟: "<label class='input'><i class='icon-prepend fas fa-link'></i><input type='text' name='bi_link[<?php echo $i; ?>]' value='<?php echo $list[$i]['bi_link']; ?>'><i></i></label><label for='bi_target_<?php echo $i; ?>' class='select' style='margin-top:5px;'><select name='bi_target[<?php echo $i; ?>]' id='bi_target_<?php echo $i; ?>'><option value=''>선택</option><option value='_blank' <?php echo $list[$i]['bi_target'] == '_blank' ? 'selected':''; ?>>새창</option><option value='_self' <?php echo $list[$i]['bi_target'] == '_self' ? 'selected': ''; ?>>현재창</option></select><i></i></label>",
	        노출수: "<?php echo $list[$i]['bi_exposed']; ?>",
	        클릭수: "<a href='<?php echo G5_ADMIN_URL; ?>/?dir=theme&amp;pid=ebbanner_itemhit&amp;bn_code=<?php echo $list[$i]['bn_code']; ?>&amp;bi_no=<?php echo $list[$i]['bi_no']; ?>&amp;wmode=1' onclick='eb_modal(this.href,\"EB배너 조회수 집계\"); return false;' class='btn-e btn-e-purple btn-e-md' style='width:100%;'><?php echo $list[$i]['bi_clicked']; ?></a>",
	        클릭율: "<?php echo $list[$i]['bi_ratio']; ?>%",
