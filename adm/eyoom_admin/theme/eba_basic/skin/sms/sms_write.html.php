 ...
 * @file    ~/theme/THEME_NAME/skin/sms/sms_write.html.php
 */
...
<style>
#sms5_send {position:relative;margin:0;padding:0;border:0;height:auto}
#sms5_send #send_emo {position:relative;top:inherit;left:inherit;margin:0;padding:10px;border:0;width:auto;border:1px solid var(--tbc-higher)}
#sms5_send #send_write {position:relative;top:inherit;left:inherit;margin:0;padding:10px !important;border:0;width:auto;height:inherit;min-height:740px;border:1px solid var(--tbc-higher)}
#sms5_send #send_book {margin:0;padding:10px;border:0;height:inherit;min-height:740px;border:1px solid var(--tbc-higher)}
#sms5_send #num_book {height:575px}
#sms5_send #send_write .sms5_box {height:110px;margin:10px 0;padding:10px 50px 10px 10px}
#sms5_send #sms_byte {top:-25px;font-size:.75rem}
#sms5_send #wr_message_lbl {top:43px;left:20px}
#sms5_send .sms5_box .box_square {width:100%;margin-bottom:0}
#sms5_send .write_scemo_btn {font-size:.75rem}
#sms5_send #write_emo_btn {top:60px}
#sms5_send .write_scemo_btn {line-height:1.2}
#sms5_send .write_scemo {left:100%}
#sms5_send .write_inner {left:inherit;padding:10px 0;width:inherit;border-color:var(--tbc-higher)}
#sms5_send #send_write h2 {font-size:.9375rem}
#sms5_send .write_floater {position:absolute;top:10px;right:10px;font-size:.9375rem}
#sms5_send .write_floater_checkbox {position:absolute;top:3px;right:0}
#sms5_send #recv_add label.input {height:inherit}
#sms5_send #recv_add .frm_input {width:150px;height:38px;line-height:inherit}
#sms5_send #recv_add button {border:0;background-color:#353535;color:#fff;height:80px;border-radius:3px}
#sms5_send #write_rsv {line-height:inherit}
#sms5_send #write_rsv select {height:38px;width:100px}
#sms5_send .btn_confirm {position:relative;overflow:hidden;margin:10px 0 0}
#sms5_send #book_desc {margin:0}
#sms5_send .td_mngsmall button.btn_frmline, #sms5_send .td_mng button.btn_frmline {background-color:#3949ab;border:0;color:#fff;padding:0 12px}
#sms5_send .btn_list .btn {height:25px;border:0;border-radius:3px;padding:0 12px;font-weight:400;font-size:.75rem;vertical-align:middle;background-color:#757575;color:#fff}
#sms5_send #sms_person_form {margin:20px 0 0}
#sms5_send #send_book .btn_submit {height:24px;border:0;border-radius:3px;padding:0 12px;font-weight:400;font-size:.75rem;vertical-align:middle;background-color:#757575;color:#fff}
@media (max-width:991px) {
    #sms5_send #send_book {min-height:inherit}
    #sms5_send #num_book {height:auto}
}
/* 테이블 */
table {clear:both;width:100%;border-collapse:collapse;border-spacing:0}
table caption {height:0;font-size:0;line-height:0;overflow:hidden}
table td{line-height:22px}
tfoot th, tfoot td {border:1px solid var(--tbc-default);padding:5px;text-align:center;font-weight:700}
.tbl_wrap {margin:0 0 10px;padding:0}
/* thead 한 줄 테이블 */
.tbl_head01 table {clear:both;width:100%;border-collapse:collapse;border-spacing:0}
.tbl_head01 thead th {border:1px solid var(--tbc-default);font-weight:normal;text-align:center;padding:8px 5px;font-weight:700}
.tbl_head01 thead input {vertical-align:top} /* middle 로 하면 게시판 읽기에서 목록 사용시 체크박스 라인 깨짐 */
.tbl_head01 thead a {text-decoration:underline}
.tbl_head01 tbody th{border:1px solid var(--tbc-default);padding:5px;text-align:center}
.tbl_head01 tbody td{border:1px solid var(--tbc-default);padding:5px;text-align:center}
</style>

<div class="admin-sms-write">
    <div class="cont-text-bg m-b-20">
        <p class="bg-info">회원정보 최근 업데이트 : <?php echo $sms5['cf_datetime']?></p>
    </div>
    ...
    <div id="sms5_send">
        <div class="row row-g-20">
            <div class="col-lg-3 lg-m-b-20">
                <div id="send_emo">
                    <div class="adm-headline">
                        <h3>이모티콘 목록</h3>
                    </div>
                    <?php include_once(EYOOM_ADMIN_CORE_PATH.'/sms/sms_write_form.php'); ?>
                </div>
            </div>
            ...
            <div class="col-lg-3 lg-m-b-20">
                <div id="send_write">
                    ...
                    <form name="form_sms" id="form_sms" method="post" action="<?php echo $action_url; ?>" onsubmit="return sms5_chk_send(this);" class="eyoom-form">
                    ...
                    <div id="write_reply">
                        <label for="wr_reply" class="label">회신<strong class="sound_only"> 필수</strong></label>
                        <label class="input required-mark">
                            <input type="text" name="wr_reply" value="<?php echo $sms5['cf_phone']?>" id="wr_reply" required size="17" maxlength="20" readonly="readonly">
                        </label>
                    </div>
                    ...
                    <div id="write_recv" class="write_inner">
                        ...
                        <div id="recv_add">
                            ...
                            <label class="input">
                                <input type="text" name="hp_name" id="hp_name" class="frm_input" size="11" maxlength="20" onkeypress="if(event.keyCode==13) document.getElementById('hp_number').focus();" placeholder="이름">
                            </label><br>
                    ...
                    <div id="write_rsv" class="write_inner">
                        ...
                        <div class="write_floater_checkbox">
                            <label class="checkbox"><input type="checkbox" name="wr_booking" id="wr_booking" onclick="booking(this.checked)"><i></i>예약전송 </span>사용</label>
                        </div>
                    ...
                    <div class="btn_confirm01 btn_confirm">
                        <input type="submit" value="전송" class="btn-e btn-e-lg btn-e-crimson btn-e-block">
            ...
            <div class="col-lg-6">
                <div id="send_book">
                    ...
                    <div id="book_tab">
                        <a href="#book_group" id="book_group" class="btn-e btn-e-dark">그룹</a>
                        <a href="#book_person" id="book_person" class="btn-e btn-e-dark">개인</a>
                        <a href="#book_level" id="book_level" class="btn-e btn-e-dark">권한</a>
                    </div>
...
function overlap_check()
{
    ...
    (function($){
        ...
        $.ajax({
            url: '<?php echo G5_ADMIN_URL; ?>/sms_admin/sms_write_overlap_check.php',
            cache:false,
            timeout : 30000,
            dataType:"html",
            data:params,
...
function booking(val)
{
    if (val)
    {
        ...
        $('#wr_by, #wr_bm, #wr_bd, #wr_bh, #wr_bi').closest('.select').removeClass('state-disabled');
    }
    else
    {
        ...
        $('#wr_by, #wr_bm, #wr_bd, #wr_bh, #wr_bi').closest('.select').addClass('state-disabled');
    }
}
...
(function($){
    ...
    sms_obj.person_select = function( params, type ){
        emoticon_list.loading(sms_obj.el_box, "<?php echo G5_ADMIN_URL; ?>/sms_admin/img/ajax-loader.gif" ); //로딩 이미지 보여줌
        $.ajax({
            url: "<?php echo G5_ADMIN_URL; ?>/sms_admin/ajax.sms_write_person.php",
            cache:false,
            timeout : 30000,
            dataType:type,
            data:params,
    ...
    $("#book_level").bind("click", function(e){
        ...
        emoticon_list.loading(sms_obj.el_box, "<?php echo G5_ADMIN_URL; ?>/sms_admin/img/ajax-loader.gif" ); //로딩 이미지 보여줌
        $.ajax({
            url: "<?php echo G5_ADMIN_URL; ?>/sms_admin/ajax.sms_write_level.php",
            cache:false,
            timeout : 30000,
            dataType:'json',
    ...
    $("#book_group").bind("click", function(e){
        ...
        emoticon_list.loading(sms_obj.el_box, "<?php echo G5_ADMIN_URL; ?>/sms_admin/img/ajax-loader.gif" ); //로딩 이미지 보여줌
        $.ajax({
            url: "<?php echo G5_ADMIN_URL; ?>/sms_admin/ajax.sms_write_group.php",
            cache:false,
            timeout : 30000,
            dataType:'html',
            success: function(data) {
