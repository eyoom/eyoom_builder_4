 ...
 * @file    ~/theme/THEME_NAME/skin/theme/ebbanner_list.html.php
 */
...
<div class="admin-ebbanner-list">
    ...
    <div class="adm-headline adm-headline-btn">
        <h3>EB배너 관리</h3>
        <?php if (!$wmode) { ?>
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=theme&pid=ebbanner_form&amp;thema=<?php echo $this_theme; ?>" class="btn-e btn-e-md btn-e-crimson adm-headline-btn"><i class="las la-plus m-r-7"></i><span>EB배너 마스터추가</span></a>
        <?php } ?>
    </div>
    ...
    <div class="f-s-13r m-b-5">
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=<?php echo $dir; ?>&amp;pid=<?php echo $pid; ?>">[전체목록]</a><span class="m-l-10 m-r-10 text-light-gray">|</span>EB배너 마스터 <?php echo number_format($total_count); ?>개
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        <th>
                            ...
                            <input type="hidden" name="bn_no[<?php echo $i; ?>]" value="<?php echo $list[$i]['bn_no']; ?>">
                            <input type="hidden" name="bn_code[<?php echo $i; ?>]" value="<?php echo $list[$i]['bn_code']; ?>">
                        </th>
                        <td class="text-center">
                            <a href="<?php echo G5_ADMIN_URL; ?>/?dir=theme&amp;pid=ebbanner_form&amp;bn_code=<?php echo $list[$i]['bn_code']; ?>&amp;w=u"><u>수정</u></a>
                        </td>
                        <td><?php echo get_text($list[$i]['bn_subject']); ?></td>
                        <td>
                            <div class="eb-clipboard"><div id="subs_code_<?php echo $i; ?>" class="eb-clipboard-cont"><?php echo $list[$i]['bn_chg_code']; ?></div><div class="eb-clipboard-btn" data-clipboard-target="#subs_code_<?php echo $i; ?>">코드복사</div></div>
                        </td>
                        <td>
                            <label class="select width-200px"><select name="bn_state[<?php echo $i; ?>]" id="bn_state_<?php echo $i; ?>"><option value="">선택</option><option value="1" <?php echo  $list[$i]['bn_state'] == '1' ? 'selected':''; ?>>보이기</option><option value="2" <?php echo  $list[$i]['bn_state'] == '2' ? 'selected':''; ?>>숨기기</option></select><i></i></label>
                        </td>
                        <td class="text-center"><?php echo substr($list[$i]['bn_regdt'], 0, 10); ?></td>
                    </tr>
                    <?php } ?>
