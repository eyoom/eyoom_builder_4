 ...
 * @file    ~/theme/THEME_NAME/skin/theme/eblatest_list.html.php
 */
...
<div class="admin-eblatest-list">
    ...
    <div class="adm-headline adm-headline-btn">
        <h3>EB최신글 - 마스터관리</h3>
        <?php if (!$wmode) { ?>
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=theme&pid=eblatest_form&amp;thema=<?php echo $this_theme; ?>" class="btn-e btn-e-md btn-e-crimson adm-headline-btn"><i class="las la-plus m-r-7"></i><span>EB최신글 마스터추가</span></a>
        <?php } ?>
    </div>
    ...
    <div class="f-s-13r m-b-5">
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=<?php echo $dir; ?>&amp;pid=<?php echo $pid; ?>">[전체목록]</a><span class="m-l-10 m-r-10 text-light-gray">|</span>EB최신글 마스터 <?php echo number_format($total_count); ?>개
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        <th>
                            ...
                            <input type="hidden" name="el_no[<?php echo $i; ?>]" value="<?php echo $list[$i]['el_no']; ?>">
                            <input type="hidden" name="el_code[<?php echo $i; ?>]" value="<?php echo $list[$i]['el_code']; ?>">
                        </th>
                        <td class="text-center">
                            <a href='<?php echo G5_ADMIN_URL; ?>/?dir=theme&amp;pid=eblatest_form&amp;el_code=<?php echo $list[$i]['el_code']; ?>&amp;w=u'><u>수정</u></a>
                        </td>
                        <td><?php echo get_text($list[$i]['el_subject']); ?></td>
                        <td>
                            <div class="eb-clipboard"><div id="subs_code_<?php echo $i; ?>" class="eb-clipboard-cont"><?php echo $list[$i]['el_chg_code']; ?></div><div class="eb-clipboard-btn" data-clipboard-target="#subs_code_<?php echo $i; ?>">코드복사</div></div>
                        </td>
                        <td>
                            <label class="select width-200px"><select name="el_state[<?php echo $i; ?>]" id="el_state_<?php echo $i; ?>"><option value="">선택</option><option value="1" <?php echo  $list[$i]['el_state'] == '1' ? 'selected':''; ?>>보이기</option><option value="2" <?php echo  $list[$i]['el_state'] == '2' ? 'selected':''; ?>>숨기기</option></select><i></i></label>
                        </td>
                        <td class="text-center"><?php echo substr($list[$i]['el_regdt'], 0, 10); ?></td>
                    </tr>
                    <?php } ?>
