<?php
/**
 * @file    /adm/eyoom_admin/core/shop/configformupdate.php
 */
 ...
 * 모달창에서 유형별 상품설정 변경
 */
if ($_POST['amode'] == 'ittype') {
    $sql = "update {$g5['g5_shop_default_table']} set {$cfg_upset}";
    sql_query($sql);
    
    alert('유형별 상품진열 설정을 적용하였습니다..', G5_ADMIN_URL . "/?dir=shop&amp;pid=configform&amp;wmode=1&amp;amode=ittype");
    
} else {
    
    // 대표전화번호 유효성 체크
    if(! (isset($_POST['de_admin_company_tel']) && $_POST['de_admin_company_tel']) )
        alert('테마설정관리 > 기본정보에서 대표전화번호를 올바르게 입력해 주세요.');
