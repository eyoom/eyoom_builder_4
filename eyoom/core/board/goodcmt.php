...
if($_POST['js'] == "on") {
    ...
    {
        $cmt = sql_fetch("select * from {$g5['write_prefix']}{$bo_table} where wr_id='{$c_id}' limit 1 ", false);
        ...
        if (isset($row['bg_flag']) && $row['bg_flag'])
        {
            ...
            $error = "이미 $status 하신 댓글 입니다.";
            print_result($error, $count);
        }
        else
        {
            // 추천(찬성), 비추천(반대) 카운트 증가
            sql_query(" update {$g5['write_prefix']}{$bo_table} set wr_{$good} = wr_{$good} + 1 where wr_id = '{$c_id}' ");
            // 내역 생성
            sql_query(" insert {$g5['board_good_table']} set bo_table = '{$bo_table}', wr_id = '{$c_id}', mb_id = '{$member['mb_id']}', bg_flag = '{$good}', bg_datetime = '".G5_TIME_YMDHIS."' ");

            $sql = " select wr_{$good} as count from {$g5['write_prefix']}{$bo_table} where wr_id = '$c_id' ";
            $row = sql_fetch($sql);
