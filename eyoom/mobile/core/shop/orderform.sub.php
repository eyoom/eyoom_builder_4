<?php
/**
 * core file : /eyoom/mobile/core/shop/orderform.sub.php
 */
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	$sod_list[$i]['sum_point'] = $sum['point'];
	$sod_list[$i]['cp_count'] = $cp_count;
	$sod_list[$i]['ct_send_cost'] = $ct_send_cost;
	$sod_list[$i]['it_options'] = $it_options;

    $tot_point      += $point;
    $tot_sell_price += $sell_price;
} // for 끝
$sod_count = count((array)$sod_list);
 ...
 * 이윰 테마파일 출력
 */
include_once(EYOOM_THEME_SHOP_SKIN_PATH.'/orderform.sub.mobile.skin.html.php');
