...
if (!$is_search_bbs) {
    ...
    // 게시물 상단고정
    if (!$page || $page == 1) {
        $fixed_where = " bo_table='{$bo_table}' and bf_open='y' and ex_datetime > '".G5_TIME_YMDHIS."' ";
        $sql = "select count(*) as cnt from {$g5['eyoom_wrfixed']} where {$fixed_where} ";
        $row = sql_fetch($sql);
        ...
        if ($row['cnt'] > 0) {
            ...
            for ($k=0; $rows=sql_fetch_array($result); $k++) {
                ...
                $list[$i] = get_list($row, $board, $board_skin_url, G5_IS_MOBILE ? $board['bo_mobile_subject_len'] : $board['bo_subject_len']);
                $list[$i]['is_wrfixed'] = true;
                $list[$i]['num'] = $total_count - ($page - 1) * $list_page_rows - $notice_count + $j;
                $i++;
                $j--;
            }
        }
...
if ($is_search_bbs) {
    $sql = " select distinct wr_parent from {$write_table} where {$sql_search} {$sql_approval} {$sql_sca} {$sql_order} limit {$from_record}, $page_rows ";
} else {
    $sql = " select * from {$write_table} where wr_is_comment = 0 {$sql_approval} {$sql_sca} ";
    if(!empty($notice_array))
